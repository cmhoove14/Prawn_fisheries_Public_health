...
snail_constant_prawn = function(t, n, parameters) {  
  with(as.list(parameters),{
    ...
    psiS1 = (alpha*(S1/A)) / (1 + alpha*handle*(N/A))
    psiS2 = (alpha*(S2/A)) / (1 + alpha*handle*(N/A))
    psiS3 = (alpha*(S3/A)) / (1 + alpha*handle*(N/A))
    psiE1 = (alpha*(E1/A)) / (1 + alpha*handle*(N/A))
    psiE2 = (alpha*(E2/A)) / (1 + alpha*handle*(N/A))
    psiE3 = (alpha*(E3/A)) / (1 + alpha*handle*(N/A))
    psiI2 = (alpha*(I2/A)) / (1 + alpha*handle*(N/A))
    psiI3 = (alpha*(I3/A)) / (1 + alpha*handle*(N/A))
...
area = 10000 #1 hectare = 10000m^2
t.consP = seq(0,365*10,1)
...
par.consP=c(
  ...
  muN1 = 1/40,       # Natural mortality rate of small snails (deaths/snail/day; assume mean lifespan = 40 days)
  muN2 = 1/50,       # Natural mortality rate of medium snails (deaths/snail/day; assume mean lifespan = 50 days)
  muN3 = 1/60,       # Natural mortality rate of large snails (deaths/snail/day; assume mean lifespan = 60 days)
  ...
  beta = 4e-6,       # Human-to-snail infection probability in reference area (infected snails/miracidia/snail/day); adjusted from Sokolow et al. 2015 (original value: 4e-6)
...
nstart.consP = c(S1 = sn.eqbm$S1, S2 = sn.eqbm$S2, S3 = sn.eqbm$S3, 
                 E1 = sn.eqbm$E1, E2 = sn.eqbm$E2, E3 = sn.eqbm$E3, 
                 I2 = sn.eqbm$I2, I3 = sn.eqbm$I3, W = sn.eqbm$W) 
sn.consP = as.data.frame(ode(nstart.consP,t.consP,snail_constant_prawn,par.consP))

  plot(sn.consP$time, sn.consP$W, type = 'l', xlab = 'time', ylab = 'W', lwd = 2)