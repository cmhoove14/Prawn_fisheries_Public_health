...
profit.lhsprcc = ggplot(profit.pcc.df, aes(x = var, y = original)) +
  ...
  labs(x = 'Parameter', 
       y = "Ten-year Cumulative profit PRCC")
...
epi.lhsprcc <- epi.W.pcc.df %>% bind_rows(., epi.N.pcc.df) %>% 
  ggplot(aes(x = var, y = original, fill = outcome)) +
    ...
    annotate('text', x = 2, y = 0.8, label = "B)", size = 8)  +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = 'Parameter', y = 'Epi Model Outcomes PRCC')
...
```{r combined_NA_analyze, echo = FALSE}
com_sens_sims %>% select(-c(A, H, siteS1:siteI3, psi1:psi3, k.ros:delta)) %>% 
...
com.lhsprcc <- com.W.pcc.df %>% bind_rows(., com.N.pcc.df) %>% 
  ggplot(aes(x = var, y = original, fill = outcome)) +
    ...
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = 'Parameter', y = 'Combined Model Outcomes PRCC')
...
prcc_layout = matrix(c(1,2,3,3), ncol = 2, byrow = T)
