...
eum_sims_ros %>% filter(P_nought == 2500) %>% 
...
profit.pcc = pcc(X = lhcpars.aqua,
                 y = as.data.frame(eum_sims_ros %>% filter(P_nought == 2500) %>% pull(cum_profits)/1000),
                 rank = TRUE)

profit.pcc.df = profit.pcc$PRCC %>% 
  mutate(cor = cor(lhcpars.aqua, eum_sims_ros %>% filter(P_nought == 2500) %>% pull(cum_profits)/1000)[,1],
         var = c(colnames(lhcpars.aqua))) %>% 
  filter(var != "k")
