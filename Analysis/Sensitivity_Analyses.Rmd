...
max(cost_sims_vol %>% filter(cum_profits > 0) %>% pull(fc))
max(cost_sims_ros %>% filter(cum_profits > 0) %>% pull(fc))

prawn_cost_plot <- cost_sims_vol %>% bind_rows(., cost_sims_ros) %>% 
  ggplot(aes(x = P_nought/1000, y = fc, fill = cum_profits, z = cum_profits)) + 
    geom_tile() +
    geom_contour(breaks = c(0), col = "white") +
    scale_fill_viridis(option = "magma", direction = 1) +
    geom_dl(aes(label = ..level..), method = "top.pieces",
            stat = "contour", breaks = c(0), col = "white") +
    scale_x_continuous(limits = range(cost_sim$P_nought/1000),
                       breaks = c(0.5, 2.5,5.0,7.5)) +
...
ggsave("Figures/Pub_Figs/prawn_fixed_cost_sensitivity.pdf",
...
max(mort_sims_vol %>% filter(cum_profits > 0) %>% pull(muP)) / par.aqua["muP"]
max(mort_sims_ros %>% filter(cum_profits > 0) %>% pull(muP)) / par.aqua["muP"]

prawn_mort_plot <- mort_sims_vol %>% bind_rows(., mort_sims_ros) %>%   
  ggplot(aes(x = P_nought/1000, y = muP, fill = cum_profits, z = cum_profits)) + 
    geom_tile() + #coord_equal() + 
    geom_contour(breaks = c(0), col = "white") +
    scale_fill_viridis(option = "magma", direction = 1) +
    #geom_dl(aes(label = ..level..), method = "top.pieces",
    #        stat = "contour", breaks = c(0)) +
    scale_x_continuous(limits = range(mort_sim$P_nought/1000),
                       breaks = c(0.5, 2.5,5.0,7.5)) +
...
ggsave("Figures/Pub_Figs/prawn_mortality_sensitivity.pdf",
       height = 6, width = 6, units = "in", dpi = 300)
...
snail.ros = sim.ros %>% dplyr::select(time, S.t, E.t, I.t, N.t) %>% 
  ...
  ggplot(aes(x = time, y = density, col = Infection)) +
              ...
              theme(legend.position = c(0.3, 0.5),
                    ...
                    title = element_text(size = 12))  +   #increase title size
              geom_line(size = 1.25) +
              scale_color_manual(values = c("Total"="black", 
                                            "Susceptible"="green", 
                                            "Exposed"="orange", 
                                            "Infected"="red")) +
              labs(y = expression(paste("Snail density (", 'N'[i], 'm'^'-2', ")"))) +
              scale_y_continuous(breaks = seq(0,50,10),
                                 labels = seq(0,50,10),
                                 limits = c(-0.1, 51)) +
              geom_vline(xintercept = 365*(years+1), lty = 2) +
              annotate('text', x = 365, y = 48, label = expression(bold('a')), size = 10) +
              guides(col = guide_legend(title="Snail \nPopulation")) +
              ggtitle("Holling's III, migration")
...
# Plot worm burden over time with rosenbergii stocking events  ###########
  w.ros.nomig = sim.ros.nomig %>% dplyr::select(time, W, Wt, Wu) %>% 
    ...
    ggplot(aes(x = time, y = burden, lty = Population)) +
            theme_bw() +
            theme(legend.position = "none",
                  axis.title=element_blank(),         #Suppress x axis
                  axis.text=element_blank(),          #Suppress x axis
                  title = element_text(size = 15)) +    #increase title size
            geom_line(col = 'purple') +
            labs(x = 'time (years)', y = expression(italic('W'))) +
            scale_y_continuous(breaks = seq(0,80,10),
...
snail.ros.nomig = sim.ros.nomig %>% dplyr::select(time, S.t, E.t, I.t, N.t) %>% 
  ...
  ggplot(aes(x = time, y = density, col = Infection)) +
              ...
              scale_x_continuous(breaks = seq(365, 365*(years*2+1), 365*5),
                                 labels = seq(365, 365*(years*2+1), 365*5)/365-1)   +
              annotate('text', x = 365, y = 48, label = expression(bold('b')), size = 10) +
              ggtitle("Holling's III, no migration")
...
snail.ros.h2 = sim.ros.h2 %>% dplyr::select(time, S.t, E.t, I.t, N.t) %>% 
  ...
  ggplot(aes(x = time, y = density, col = Infection)) +
              ...
              scale_y_continuous(breaks = seq(0,50,10),
                                 ...
                                 limits = c(-0.1, 51)) +
              geom_vline(xintercept = 365*(years+1), lty = 2) +
              annotate('text', x = 365, y = 48, label = expression(bold('c')), size = 10) +
              ggtitle("Holling's II, migration") +
              scale_x_continuous(breaks = seq(365, 365*(years*2+1), 365*5),
                                 labels = seq(365, 365*(years*2+1), 365*5)/365-1)
...
w.ros.nomig.h2 = sim.ros.nomig.h2 %>% dplyr::select(time, W, Wt, Wu) %>% 
    ...
    ggplot(aes(x = time, y = burden, lty = Population)) +
            ...
            theme(legend.position = "none",
                  axis.text = element_text(size = 12),  #increase axis label size
                  axis.title = element_text(size = 15), #increase axis title size
                  axis.title.y=element_blank(),         #Suppress x axis
                  axis.text.y=element_blank(),          #Suppress x axis
                  title = element_text(size = 15)) +    #increase title size
            geom_line(col = 'purple') +
            labs(x = 'time (years)', y = expression(italic('W'))) +
            scale_y_continuous(breaks = seq(0,80,10),
...
snail.ros.nomig.h2 = sim.ros.nomig.h2 %>% dplyr::select(time, S.t, E.t, I.t, N.t) %>% 
  ...
  ggplot(aes(x = time, y = density, col = Infection)) +
              theme_bw() +
              theme(legend.position = "none",
                    axis.text.x = element_text(size = 10),  #increase axis label size
                    axis.title.x = element_text(size = 12), #increase axis title size
                    axis.title.y=element_blank(),         #Suppress x axis
                    axis.text.y=element_blank(),          #Suppress x axis
                    title = element_text(size = 12))  +   #increase title size
              geom_line(size = 1.25) +
              ...
              scale_y_continuous(breaks = seq(0,50,10),
                                 ...
                                 limits = c(-0.1, 51)) +
              geom_vline(xintercept = 365*(years+1), lty = 2) +
              annotate('text', x = 365, y = 48, label = expression(bold('d')), size = 10) +
              ggtitle("Holling's II, no migration") +
              scale_x_continuous(breaks = seq(365, 365*(years*2+1), 365*5),
                                 labels = seq(365, 365*(years*2+1), 365*5)/365-1)
...
```{r save_holling_mig_fig, include = FALSE, echo = FALSE}
png("Figures/snail_predation_migration_fig.png",
...
pdf("Figures/Pub_Figs/snail_predation_migration_fig.pdf",
...
ggsave("Figures/Pub_Figs/prawn_sensitivity_monotonicity.pdf",
       height = 10.5, width = 8.5, units = "in", dpi=300)
...
#PRCC plot 
profit_var_lvls <- profit.pcc.df$var[order(abs(profit.pcc.df$original))]

profit.lhsprcc <- profit.pcc.df %>% select(-cor) %>% 
  mutate(var = factor(var, levels = profit_var_lvls)) %>% 
  ...
  annotate('text', x = 11, y = -0.6, label = expression(bold('b')), size = 10) +
  labs(x = 'Parameter', 
...
#Use simulations over parameter sets from above
epi_w_monotonicity <- epi_sens_sims %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr)) %>% 
...
```{r epi_monotonicity_snails, echo=FALSE, fig.height=10.5, fig.width=8}  
#Use simulations over parameter sets from above

epi_It_monotonicity <- epi_sens_sims %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr)) %>% 
  gather("parameter", "value", f:muH) %>% 
...
ggsave("Figures/Pub_Figs/epi_w_monotonicity.pdf",
       height = 10.5, width = 8.5, units = "in", dpi = 300)
...
ggsave("Figures/Pub_Figs/epi_I_monotonicity.pdf",
       height = 10.5, width = 8.5, units = "in", dpi = 300)
...
epi.W.pcc = pcc(X = lhcpars.epi %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr)),
                y = as.data.frame(epi_sens_sims %>% pull(W)),
                rank = TRUE)

epi.W.pcc.df = epi.W.pcc$PRCC %>% 
  mutate(cor = cor(lhcpars.epi %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr)), 
                   epi_sens_sims %>% pull(W)),
         var = colnames(lhcpars.epi %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr))),
         outcome = "W")

# Epi infected snail density sensitivity   #############
epi.I.pcc = pcc(X = lhcpars.epi %>% select(-c(A, H, cvrg, xi:siteI3, psi1:psi3, nfr)),
...
# PRCC plot #######
epi_var_lvls <- epi.W.pcc.df$var[order(abs(epi.W.pcc.df$original))]

epi.lhsprcc <- epi.W.pcc.df %>% bind_rows(., epi.I.pcc.df) %>% select(-cor) %>% 
  mutate(var = factor(var, levels = epi_var_lvls)) %>% 
  ggplot(aes(x = var, y = original, fill = outcome)) +
    ...
    annotate('text', x = 18, y = -0.6, label = expression(bold("c")), size = 10)  +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = 'Parameter', y = 'PRCC') + coord_flip()
...
Assess sensitivity of Cumulative DALYs in the integrated prawn-MDA treatment to all parameters  

```{r combined_sens_sims, fig.height=10.5, fig.width=8.5}
...
comp_ints <- comp_ints %>% 
  ...
  mutate(DALYs_Averted = None - Both,
         DALYs_Averted_MDA = None - MDA,
         DALYs_Averted_Prawns = None - Prawns,
         Class = ifelse(DALYs_Averted_MDA > DALYs_Averted_Prawns,
                        "MDA_Superior", "Prawns_Superior"))

#Get variable levels to sort in figure:
# want to sort by difference in median centered parameter value
comp_var_lvls <- bind_cols(as.data.frame(scale(as.matrix(comp_ints %>% 
                                             select(-c(None, MDA, Prawns, Both, 
                                                       siteS1:siteI3, psi1:psi3, 
                                                       A, H, fc, Class, eta,
                                                       DALYs_Averted, 
                                                       DALYs_Averted_Prawns,
                                                       DALYs_Averted_MDA))))),
                           comp_ints %>% select(DALYs_Averted_Prawns,
                                                DALYs_Averted_MDA)) %>% 
  filter(!is.na(DALYs_Averted_MDA)) %>% 
  gather("variable", "value", -c(DALYs_Averted_Prawns, DALYs_Averted_MDA)) %>% 
  mutate(mda_superior = ifelse(DALYs_Averted_MDA > DALYs_Averted_Prawns, 1, 0)) %>% 
  group_by(mda_superior, variable) %>% 
  summarise(med_mda = median(value, na.rm = TRUE)) %>% 
  spread(key = mda_superior, value = med_mda) %>% 
  mutate(diff = `1`-`0`) %>% arrange(diff) %>% pull(variable)

comp_var_lvls <- factor(comp_var_lvls, levels = comp_var_lvls)

comp_pars_mda_prawns <- bind_rows(scale(as.matrix(comp_ints %>% 
          filter(DALYs_Averted_Prawns > DALYs_Averted_MDA) %>% 
          select(-c(None, MDA, Prawns, Both, 
                    siteS1:siteI3, psi1:psi3, A, H, fc, Class, eta,
                    DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns)))) %>% 
          as.data.frame(.) %>% 
          mutate(Class = "Prawns_Superior"),
          scale(as.matrix(comp_ints %>% 
          filter(DALYs_Averted_MDA > DALYs_Averted_Prawns) %>% 
          select(-c(None, MDA, Prawns, Both, 
                    siteS1:siteI3,psi1:psi3, A, H, fc, Class, eta,
                    DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns)))) %>% 
          as.data.frame(.) %>% 
          mutate(Class = "MDA_Superior")) %>% 
  reshape2::melt(.) %>% 
  mutate(variable = factor(variable, levels = comp_var_lvls)) %>% 
  ggplot(aes(x = variable, value, col = Class)) + geom_boxplot() +
    theme_bw() + 
    theme(axis.text = element_text(size = 12),  #increase axis label size
          axis.title = element_text(size = 15), #increase axis title size
          axis.title.y = element_blank(),
    ...
    scale_x_discrete(labels = c("a.p" = expression(italic(a[P])),
                                ...
                                "theta2" = expression(italic(theta[2])),
                                "z" = expression(italic(z)))) +
    ylab("Centered parameter value") +
    ggtitle("Centered distributions of parameters",
            subtitle = "stratified by superior performance of MDA- or Prawn-only interventions")


comp_pars_mda_prawns

```

```{r save_comp_pars, include=FALSE, echo = FALSE}
comp_pars_mda_prawns

ggsave("Figures/comp_pars_mda_prawns.png",
       height = 10.5, width = 8.5, units = "in")

comp_pars_mda_prawns

ggsave("Figures/Pub_Figs/comp_pars_mda_prawns.pdf",
       height = 10.5, width = 8.5, units = "in", dpi = 300)

```


### Monotonicity of cumulative DALYs under integrated intervention to parameter uncertainty  
...
dalys_monotonicity <- comp_ints %>% select(-c(A, H, k, fc, eta, siteS1:siteI3, psi1:psi3)) %>% 
  ...
  ggplot(aes(x = value, y = Both)) + 
    theme_bw() + labs(y = "Cumulative DALYs") +
    geom_point(size = 0.5) +
...
ggsave("Figures/Pub_Figs/dalys_monotonicity.pdf",
       height = 10.5, width = 8.5, units = "in", dpi = 300)

```

```{r combined_prcc, fig.height=10, fig.width=8}
# Combined model worm burden sensitivity
com.DALYs.pcc = pcc(X = comp_ints %>% 
                      filter(!is.na(Both)) %>% 
                      select(-c(DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns,
                                Both, None, MDA, Prawns,  Class,
                                A, H, k, fc, eta, epmL, siteS1:siteI3, psi1:psi3)),
                y = as.data.frame(comp_ints %>% filter(!is.na(Both)) %>% pull(Both)),
                rank = TRUE)
...
com.DALYs.pcc.df = com.DALYs.pcc$PRCC %>% 
  mutate(cor = cor(comp_ints %>% 
                     ...
                     select(-c(DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns,
                                Both, None, MDA, Prawns,  Class,
                                A, H, k, fc, eta, epmL, siteS1:siteI3, psi1:psi3)), 
                   comp_ints %>% filter(!is.na(Both)) %>% pull(Both)),
         var = colnames(comp_ints %>% 
                          select(-c(DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns,
                                    Both, None, MDA, Prawns, Class,
                                    A, H, k, fc, eta, epmL, siteS1:siteI3, psi1:psi3))),
         outcome = "Both")
...
# PRCC plot #######
dalys_var_lvls <- com.DALYs.pcc.df$var[order(abs(com.DALYs.pcc.df$original))]

com.lhsprcc <- com.DALYs.pcc.df %>% select(-cor) %>% 
  mutate(var = factor(var, levels = dalys_var_lvls)) %>% 
  ggplot(aes(x = var, y = original)) +
    theme_bw() + 
    theme(axis.text = element_text(size = 12),  #increase axis label size
          axis.title = element_text(size = 15)) + #increase axis title size 
    ...
    scale_x_discrete(labels = c("a.p" = expression(italic(a[P])),
                                ...
                                "z" = expression(italic(z)))) +
    geom_bar(stat = 'identity', width = 0.5, position = position_dodge()) +
    annotate('text', x = nrow(com.DALYs.pcc.df), y = -0.6, 
             label = expression(bold("a")), size = 10)  +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = 'Parameter', y = 'PRCC') + coord_flip()
...
ggsave("Figures/dalys_prcc.png",
       height = 10.5, width = 4.5, units = "in")

```

```{r combined_prcc_rmv0, fig.height=10, fig.width=8}
# Combined model worm burden sensitivity
com.DALYs.pcc.rmv0 = pcc(X = comp_ints %>% 
                      filter(!is.na(Both) & None > 0) %>% 
                      select(-c(DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns,
                                Both, None, MDA, Prawns,  Class,
                                A, H, k, fc, eta, epmL, siteS1:siteI3, psi1:psi3)),
                y = as.data.frame(comp_ints %>% 
                                    filter(!is.na(Both) & None > 0) %>% 
                                    pull(Both)),
                rank = TRUE)

com.DALYs.pcc.rmv0.df = com.DALYs.pcc.rmv0$PRCC %>% 
  mutate(cor = cor(comp_ints %>% 
                     filter(!is.na(Both) & None > 0) %>% 
                     select(-c(DALYs_Averted, DALYs_Averted_MDA, DALYs_Averted_Prawns,
                                Both, None, MDA, Prawns,  Class,
...
# Correlation plots #######
ggplot(com.DALYs.pcc.rmv0.df, aes(x = cor, y = original, label = var)) + 
  geom_point(pch = 16) + theme_bw() +
  geom_text(aes(label = var), hjust = -0.5, vjust = -0.5) +
  labs(x = "Correlation coefficient",
       y = "PRCC",
...
# PRCC plot #######
dalys_var_lvls_rmv0 <- com.DALYs.pcc.rmv0.df$var[order(abs(com.DALYs.pcc.rmv0.df$original))]

com.lhsprcc.rmv0 <- com.DALYs.pcc.rmv0.df %>% select(-cor) %>% 
  mutate(var = factor(var, levels = dalys_var_lvls_rmv0)) %>% 
  ggplot(aes(x = var, y = original)) +
    theme_bw() + 
    theme(axis.text = element_text(size = 12),  #increase axis label size
          axis.title = element_text(size = 15), #increase axis title size
    ...
    scale_x_discrete(labels = c("a.p" = expression(italic(a[P])),
                                ...
                                "z" = expression(italic(z)))) +
    geom_bar(stat = 'identity', width = 0.5, position = position_dodge()) +
    annotate('text', x = nrow(com.DALYs.pcc.df)-1, y = -0.6, 
             label = expression(bold("a")), size = 10)  +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = 'Parameter', y = 'Combined Model Outcomes PRCC') + coord_flip() +
    ggtitle("Correlation with no transmission parameter sets removed")

com.lhsprcc.rmv0

```

```{r combined_prcc_rmv0_save, include = FALSE, echo = FALSE}
com.lhsprcc.rmv0

ggsave("Figures/dalys_rmv0_prcc.png",
       height = 10.5, width = 4.5, units = "in")
...
```{r save_prcc_combined, include=FALSE, echo = FALSE}
png("Figures/combined_prcc_fig.png",
...
pdf("Figures/Pub_Figs/combined_prcc_fig.pdf",
    width = 8, height = 10.5)
